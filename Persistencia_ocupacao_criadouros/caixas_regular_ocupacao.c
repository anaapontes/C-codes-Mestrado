...
int main() {
    ...
    ar = fopen("mosquitos_regular.dat", "w");
    er = fopen("ocupacao_regular.dat", "w");
    

    // Loop para variar o número de caixas
    for (NC = 50; NC <= 50; NC += 20) {
        ...
        for (n = 0; n < MEDIA; n++) {
            ...
            NCC = Equilibrio(1, NC); // 0 = aleatório e 1 = uniforme
        ...
        // Exibe os resultados
        printf("NCC: %d, Mosquitos: %lf, var: %lf, \n", NCC, mean_dinU, sqrt(var));
...
int Equilibrio(int icond, int NC) {
    ...
    int sum, px, l = 4, m = 4;                  // Altera l e m manualmente
    double xe;
