...
#define ALFA 0.8                // Parâmetro de decaimento 
...
int main() {
    ...
    int i, j, k, n, nU, NC, NCC, mosquitos;
    double mean_dinU, QMC[MEDIA], var, totalPorcentagemOcupacao, totalPorcentagemFiltrada, totalPorcentagem;
    double mediaPorcentagemOcupacaoDireta, mediaPorcentagemOcupacaoFiltrada, mediaContador;
    double somaPorcentagensDiretas[MEDIA], somaPorcentagensFiltradas[MEDIA]; // Para armazenar as porcentagens em cada simulação
    double varPorcentagemOcupacaoDireta = 0, varPorcentagemFiltrada = 0;   // Variâncias
    ...
    ar = fopen("mosquitos_agrupada.dat", "w");
    er = fopen("ocupacao_agrupada.dat", "w");
    ...
    for (NC = 0; NC <= 1500; NC += 20) {
        ...
        mediaContador = 0.0;
        varPorcentagemOcupacaoDireta = 0.0;
        varPorcentagemFiltrada = 0.0;
        ...
        for (n = 0; n < MEDIA; n++) {
            ...
            // Armazena a porcentagem média de ocupação direta e filtrada
            somaPorcentagensDiretas[n] = totalPorcentagem / NCC;
            somaPorcentagensFiltradas[n] = calcularMediaPorcentagemOcupacaoFiltrada(porcentagens, NCC, &mediaContador);
        }

        figura(NCC,NCC);

        // Calcula a média das porcentagens de ocupação total e filtrada
        mediaPorcentagemOcupacaoDireta = 0.0;
        mediaPorcentagemOcupacaoFiltrada = 0.0;
        for (n = 0; n < MEDIA; n++) {
            mediaPorcentagemOcupacaoDireta += somaPorcentagensDiretas[n];
            mediaPorcentagemOcupacaoFiltrada += somaPorcentagensFiltradas[n];
        }
        mediaPorcentagemOcupacaoDireta /= MEDIA;
        mediaPorcentagemOcupacaoFiltrada /= MEDIA;
        mediaContador /= MEDIA;  // Calculando a média do contador

        // Calcula a variância das porcentagens diretas e filtradas
        for (n = 0; n < MEDIA; n++) {
            varPorcentagemOcupacaoDireta += pow(mediaPorcentagemOcupacaoDireta - somaPorcentagensDiretas[n], 2);
            varPorcentagemFiltrada += pow(mediaPorcentagemOcupacaoFiltrada - somaPorcentagensFiltradas[n], 2);
        }

        varPorcentagemOcupacaoDireta /= (MEDIA - 1);
        varPorcentagemFiltrada /= (MEDIA - 1);
        ...
        fprintf(er, "%d %lf %lf %lf %lf\n", NCC, mediaPorcentagemOcupacaoDireta, mediaPorcentagemOcupacaoFiltrada, sqrt(varPorcentagemOcupacaoDireta), sqrt(varPorcentagemFiltrada));
...
int Equilibrio(int icond, int NC) {
    int j, i, n, k, NCC, TC[3] = {CC, CC, CC}, candidatos[20], x_candidato, y_candidato, c, x_existente, y_existente, sum, px;
    double xe, distancias[20], pesos[20], soma_pesos, escolha, acumulado, soma_dist_total;
    ...
    // Inicializa a posição de todas as caixas (PA[n].POS) como -1
    for (n = 0; n < LR2; n++) PA[n].POS = -1; 

    // DISTRIBUICAO BASEADA EM DISTÂNCIAS
    n = 0;

    // Posiciona a primeira caixa aleatoriamente
    j = (int)rand() % LR;
    i = (int)rand() % LR;
    ...
    // Posiciona as caixas subsequentes
    while (n < NC) {
        soma_pesos = 0.0;  // Reinicia soma_pesos a cada nova iteração
        soma_dist_total = 0.0;

        // Sorteia 10 posições candidatas
        for (k = 0; k < 20; k++) {
            do {
                j = (int)rand() % LR;
                i = (int)rand() % LR;
                px = j + (int)i * LR;
            } while ((i < BORDAS || i >= LR - BORDAS || j < BORDAS || j >= LR - BORDAS) || (n > 0 && PA[n - 1].POS == px));

            candidatos[k] = px;
            x_candidato = px % LR;
            y_candidato = px / LR;

            // Calcula a distância média em relação a todas as caixas já posicionadas
            distancias[k] = 0.0;
            for (c = 0; c < n; c++) {
                x_existente = PA[c].POS % LR;
                y_existente = PA[c].POS / LR;
                distancias[k] += sqrt(pow(x_candidato - x_existente, 2) + pow(y_candidato - y_existente, 2));
            }
            soma_dist_total += distancias[k];  // Soma total das distâncias
        }

        // Calcula os pesos com decaimento exponencial baseado na distância
        for (k = 0; k < 20; k++) {
            pesos[k] = exp(-ALFA * distancias[k]);  // Aplica o decaimento exponencial
            soma_pesos += pesos[k];
        }

        // Escolhe a posição com base nos pesos
        escolha = (double)rand() / RAND_MAX * soma_pesos;
        acumulado = 0.0;
        for (k = 0; k < 20; k++) {
            acumulado += pesos[k];
            if (escolha <= acumulado) {
                PA[n].POS = candidatos[k];
                break;
            }
        }   
