...
#define phiI 0. //(2.1)              // Taxa de oviposição para indivíduos infectados. Valor médio estimado em 2.07 ovos por ciclo.
...
#define q 1.//0.95                  // Incompatibilidade citoplasmática, a taxa de incompatibilidade entre indivíduos.
#define xi 0.//0.98                 // Probabilidade de transmissão vertical, com referência entre 0.93 e 0.95.
...
#define TFIM2 (TFIM+60)         // Tempo final, após 30 dias sem liberação
...
int main() {
    ...
    ar = fopen("60-eficacia_pt_m_libvirgem_virgem-agrupada.dat", "w");

    QI=0.;//proporcao de infectados com relacao a nao infectados
    NC=280;// Numero de criadouros  
    while(QI<=20){
...
//---------------------------------------------------------------------------------------------
//Função que calcula a eficácia pontual
 double Eficacia(double dinU[TFIM2])
 {
   double A0,Ac,J;
   int i;

   A0=dinU[TEQ-1];   //antes do controle
   Ac=dinU[TFIM2-1]; //após o controle
